...
export const FiltrosSiniestros = () => {
  ...
  const onSubmit = (newValues, actions) => {
    console.log("IS SUBMITTING FORM", newValues);
    // if (
    //   validateTimeBetweenDates(
    //     newValues.fcCreacionDesde,
    //     newValues.fcCreacionHasta
    //   )
    // ) {
    //   alert.current.show_error(
    //     "El intervalo de fecha de creación no puede ser mayor a 1 año"
    //   );
    ...
    // if (
    //   validateTimeBetweenDates(
    //     newValues.fcIncurrenciaDesde,
    //     newValues.fcIncurrenciaHasta
    //   )
    // ) {
    //   alert.current.show_error(
    //     "El intervalo de fecha de incurrencia no puede ser mayor a 1 año"
    //   );
    //   actions?.setSubmitting(false);
    //   return;
    // }
  ...
  const fetchData = (q = {}) => {
    ...
    if (query2.page !== query.page || query2.page === 0) {
      axios
        .post(routesVam.api + "/siniestros-portal/search", query2)
        .then((resp) => {
          let data = resp.data;
          setData({
        ...
        find_ram_grupos().then((resp) => {
          ...
          find_ramos_ramgrp().then((resp) => {
            ...
            resp.forEach((r) => {
              ...
              item.nmRamo = ramos.find(
                (ramo) => ramo.cdRamo === item.cdRamo
              ).nmRamo;
              item.nmArea = areas.find(
                (area) => area.cdRamGrupo === item.cdRamGrupo
              ).nmRamGrupo;
              item.nm = item.nmRamo;
              if (isVam(item)) {
                ramosTmp.push(item);
            ...
            let ram = ramosTmp
              .toSorted((a, b) => a.nmRamo.localeCompare(b.nmRamo))
              .map((x) => ({
                value: x.cdRamo,
                label: x.nmRamo,
                nmArea: x.nmArea,
  ...
  const getEjecutivos = () => {
    axios
      .get(routesVam.api + "/siniestros-portal/ejecutivos")
      .then((resp) => {
        let list = resp.data;
        list.forEach((p) => {
          p.label = p.ejecutivo;
  ...
  const getRamosNoAdmin = async (ramos, cedula) => {
    let list = await axios.get(routesVam.api + "/siniestro/deducible/matrix", {
      params: { cedula, caducada: false },
    });
    let ramosTmp = [];
    (list.data || []).forEach((x) => {
  ...
  const addStatus = (lista) => {
    lista.forEach((elemento) => {
      const isExpired =
        calcularDias(elemento?.fcPrimeraFactura) > SINIESTRO_EXPIRED_DAYS;
      elemento.class = isExpired
        ? "tr-danger"
        : ESTADOS_CLASS.find((x) => x.value === elemento.estadoPortal)?.label;
    });
    return lista;
  };
  ...
  return (
    <Pages title={"Siniestros registrados"}>
        ...
        <div className="container-fluid">
          ...
          <div className="row">
                  ...
                  >
                    {({
                      ...
                      <div className="row gy-3">
                        ...
                        <div className="col-md-6">
                          <label className="form-label fw-bold text-secondary fs-7">
                            Fc.Incurrencia Desde:{" "}
                          </label>
                          <UqaiField
                            name="fcIncurrenciaDesde"
                            placeholder="Ingrese Fecha"
                            component={UqaiCalendario}
                          />
                        </div>
                        <div className="col-md-6">
                          <label className="form-label fw-bold text-secondary fs-7">
                            Fc.Incurrencia Hasta:{" "}
                          </label>
                          <UqaiField
                            name="fcIncurrenciaHasta"
                            placeholder="Ingrese Fecha"
                            component={UqaiCalendario}
                          />
                        </div>
                        ...
                        <div className="col-12">
                          ...
                          >
                            ...
                            {TIPOS.map((tp) => (
                              <option value={tp.value} key={tp.value}>
                                {tp.label}
                              </option>
                            ))}
                          </UqaiField>
                        ...
                        {(isAdmin || isEjecutivo) && (
                          <div className="col-12">
                            ...
                            <UqaiField
                              ...
                              component={Select}
                              options={ejecutivos}
                              value={ejecutivos.find(
                                (x) => x.value === values.cdEjecutivo
                              )}
            ...
            <div className="col-lg-9 col-md-6 col-sm-12 py-2">
              <Card className="shadow">
                <CardHeader className="d-flex flex-column flex-sm-row align-items-start align-items-sm-center border-bottom border-primary justify-content-between">
                  <h5 className="my-0 fw-bold">
                      ...
                      <ReactTable
                        ...
                        columns={[
                            ...
                            Cell: (row) => {
                              return (
                                <div>
                                  {
                                    TIPOS.find(
                                      (x) => x.value === row.original.tpReclamo
                                    )?.label
                                  }
                                </div>
                              );
                          ...
                          },
                          {
                            ...
                            Cell: (row) => {
                              return (
                                <Estados estado={row.original.estadoPortal} />
                              );
                            },
                          },
                          {
                          ...
                          },
                          {
                            ...
                            Cell: (row) => {
                              return (
                                <div>
                                  {calcularDias(row.original.fcPrimeraFactura)}
                                </div>
                              );
                            },
                            minResizeWidth: 10,
...
export const LeyendaColor = ({ color, txt }) => {
  return (
    <div style={{ display: "flex", alignItems: "center", margin: "25px" }}>
      ...
      ></div>
      <span className={"form-label text-bold text-secondary fs-6 mt-2"}>
        {txt}
      </span>
    </div>
  );

  ...
  return (
    <nav className="nav flex-column p-3">
      {items?.map((item, index) => (
        ...
        >
          <i
            className={`icon-uqai icon-uqai-lg ${item.icon} text-center mx-2`}
            style={{ width: 26 }}
          ></i>
          {item.text}
        </Link> //NOSONAR
...
export const Sidebar = ({ toggle, isOpen }) => {
  //DIsplay ejecutivoSideBard
  const user = { isEjecutivo: true };
  ...
  const linksGen = [
    ...
    {
      text: "Siniestros Vam",
      icon: "uqai-lista-siniestros-reportados",
      to: "/gen/siniestros-vam",
    },
    {
      text: "Siniestros Generales",
      icon: "uqai-siniestro-general",
      to: "/gen/siniestros-gen",
    },
  ];
  ...
  const linksVam = [
    ...
    {
      text: "Siniestros Reportados",
      icon: "uqai-siniestro-reportado",
      to: "/vam/siniestros-reportados",
    },
    {
      text: "Crear siniestro",
      icon: "uqai-crear-siniestro",
  ...
  const linksAdmin = [
    ...
    },
    {
      ...
      icon: "uqai-siniestro-general",
      to: "/gen/usuarios-gestor",
    },
    {
      text: "usuarios vam",
      icon: "uqai-siniestro-general",
      to: "/vam/usuarios-gestor",
    },
    {
      text: "Siniestros Reportados",
      icon: "uqai-siniestro-reportado",
      ...
      <Offcanvas.Body className="p-0 d-flex flex-column">
        ...
        >
          <div className="d-flex fw-regular">
            {isGen && (
              <div
                className={`flex-justified d-flex p-2 ${getTabStyle(1)}`}
                role="button"
                aria-label="Pólizas Contratantes"
                onClick={() => switchTab(1)}
              >
                <i className="icon-uqai uqai-polizas-contratantes me-2 fs-4"></i>
                <span className="lh-sm fs-7 d-flex">
                  Pólizas Contratantes{" "}
                  <UqaiTooltip
                    size={"fs-6"}
                    message={
                      "Pólizas que han sido contratadas o emitidas como dueño de la misma."
                    }
                  />
                </span>
              </div>
            ...
            {isVam && (
              <div
                className={`flex-justified d-flex p-2 ${getTabStyle(2)}`}
                role="button"
                aria-label="Pólizas Aseguradas"
                onClick={() => switchTab(2)}
              >
                ...
                <span className="lh-sm fs-7 d-flex">
                  ...
                  <UqaiTooltip
                    size={"fs-6"}
                    message={
                      "Pólizas a la que pertenece como beneficiario de un seguro dentro de una empresa."
                    }
                  />
                </span>
              </div>
            ...
            {user.isAdmin && (
              <div
                className={`flex-justified d-flex p-2 ${getTabStyle(3)}`}
                role="button"
                aria-label="Admin"
                onClick={() => switchTab(3)}
              >
                <i className="icon-uqai uqai-polizas-asegurados me-2 fs-4"></i>
                <span className="lh-sm">Admin</span>
              </div>
            )}
            {user.isEjecutivo && (
              <div
                className={`flex-justified d-flex p-2 ${getTabStyle(4)}`}
                role="button"
                aria-label="Ejecutivo"
                onClick={() => switchTab(4)}
              >
                <i className="icon-uqai uqai-polizas-asegurados me-2 fs-4"></i>
                <span className="lh-sm">Ejecutivo</span>
              </div>
            )}
          </div>

          <div
            className="flex-grow-1 sidebar-filter-gradient bg-img-center fw-regular"
            ...
            {tab === 0 && (
              <>
                <p className={"px-3 mt-3"}>
                  {" "}
                  Seleccione el menú para ver las opciones.
                </p>
              </>
            )}
          </div>
